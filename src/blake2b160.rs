...
impl Midstate {
    ...
    pub fn from_inner(inner: [u8; 20]) -> Self {
        Midstate(inner)
    }
...
impl Default for HashEngine {
    fn default() -> Self {
        HashEngine {
            buffer: [0; BLOCK_SIZE],
            blake: VarBlake2b::new(20).unwrap(),
            length: 0,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test() {
        let input = decode_hex("03f3e08754fac2440553927c5d97fa601997f46aba617efdbd3b50194d6cd415f6").expect("decoding failed");
        let output_str = "f92654e6960cb56ceff079287fd5dfefada45f7e";

        let mut raw_blake = VarBlake2b::new(20).unwrap();
        raw_blake.update(&input);
        raw_blake.finalize_variable_reset(|res| {
            assert_eq!(&res.to_hex(), &output_str);
        });

        let hash = blake2b160::Hash::hash(input.as_slice());
        assert_eq!(hash, blake2b160::Hash::from_hex(output_str).expect("parse hex"));
